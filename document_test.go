...
func ExampleDocument_GetWordAfterCursorUntilSeparator() {
	d := &Document{
		Text:           `hello,i am tengteng22,thank you for using go-prompt`,
...
func ExampleDocument_GetWordBeforeCursorUntilSeparatorIgnoreNextToCursor() {
	d := &Document{
		Text:           `hello,i am tengteng22,thank you for using go-prompt`,
		cursorPosition: len(`hello,i am tengteng22,`),
	}
...
func TestDocument_DisplayCursorPosition(t *testing.T) {
	patterns := []struct {
		...
		},
		{
			...
			// must be checked.
			// https://github.com/tengteng22/go-prompt/pull/99
			document: &Document{
				Text:           "Добрый день",
				cursorPosition: 3,
			},
